"""The hub's own documentation, served as an MCP backend.
...
It is an ordinary backend rather than something wired in at the root: it gets
the same OAuth, the same grants and the same levels as everything else, and it
is listed, granted and disabled the same way. Every tool here is read-only, so
it works at every level including `viewer`.
...
class DocsPlugin(PluginDefaults):
    id = "mcphub-docs"
    name = "mcphub documentation"
    description = (
        "This hub's own documentation for building apps and plugins for it, and "
        "deploying them with it. Nothing to configure."
    )
    fields = ()
    ...
    def build(self, instance: BackendInstance) -> MCPServer:
        ...
        def search_docs(query: str) -> str:
            ...
            For finding which page covers something — `read_topic` gives the
            whole page once you know.
            """
        ...
        for topic in topics():
            self._expose(server, topic)
        return server

    @staticmethod
    def _expose(server: MCPServer, topic: Topic) -> None:
        """Offer a page as a resource too, so a client can attach it directly.

        A function per page rather than one templated `docs://{name}` resource:
        a template is not listed, so a client offering a menu of what it can
        attach would show nothing at all.
        """
        body = topic.body
    ...
    async def check(self, instance: BackendInstance) -> CheckResult:
        ...
        return CheckResult(True, f"{len(found)} pages: "
